...
module.exports = class Tickets {
    ...
    async getInteractions(number) {
        let response = await this.client.get(
            `${this.baseResource}${number}/interactions`
        );
        return response.data;
    }
    ...
    async update(ticket) {
        let response = await this.client.put(
            `${this.baseResource}${ticket.number}`,
            ticket
        );
        return response.data;
    }
    ...
    async getCustomLists() {
        let response = await this.client.get(
            `${this.baseResource}custom-lists/`
        );
        return response.data;
    }
    ...
    async getCustomListsCounts() {
        let response = await this.client.get(
            `${this.baseResource}custom-lists/counts`
        );
        return response.data;
    }
    ...
    async getCustomList(idList) {
        let response = await this.client.get(
            `${this.baseResource}custom-list/${idList}`
        );
        return response.data;
    }
    ...
    async getCustomListCount(idList) {
        let response = await this.client.get(
            `${this.baseResource}custom-list/${idList}/count`
        );
        return response.data;
    }
    ...
    async executeList(idList, options = {}) {
        let response = await this.client.post(
            `${this.baseResource}list/${idList}/execute`,
            options
        );
        return response.data;
    }
    ...
    async getDefaultLists() {
        let response = await this.client.get(
            `${this.baseResource}default-lists`
        );
        return response.data;
    }
                ...
                if (Array.isArray(options[prop])) {
                    ...
                    operatorsMap.forEach((item) => {
                        if (
                            ...
                            Array.isArray(options[prop][item.prop])
                        ) {
    ...
    async search(options) {
        ...
        let response = await this.client.get(
            `${this.baseResource}search?${builder.queryString || ""}`
        );
        ...
        if (
            ...
            response.headers["nextpageurl"]
        ) {
            newOptions.lastTicketReference =
                response.data[response.data.length - 1].lastDateUpdate;
        }
    ...
    async attachments(attachment) {
        try {
            let response = await this.client.post(
                "https://api.octadesk.services/attachments/upload/",
                attachment
            );
